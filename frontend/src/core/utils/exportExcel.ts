...
export function exportKardexToExcel(
  data: any[],
  initialBalance: { quantity: number; value: string } | null,
  productName: string,
  from: string,
  to: string
) {
  ...
  if (initialBalance) {
    rows.push({
      Fecha: dayjs(from).format("DD/MM/YYYY"),
      Tipo: "Inicial",
      Referencia: "SALDO INICIAL",
      Entrada: "",
      Salida: "",
      "Saldo Cantidad": Number(initialBalance.quantity),
      "Saldo Valor": Number(initialBalance.value),
    });
  }
  ...
  data.forEach((row) => {
    rows.push({
      Fecha: dayjs(row.createdAt).format("DD/MM/YYYY HH:mm"),
      Tipo: row.type === "IN" ? "Entrada" : "Salida",
      Referencia: row.reference ?? "",
      Entrada: row.type === "IN" ? Number(row.quantity) : "",
      Salida: row.type === "OUT" ? Number(row.quantity) : "",
      "Saldo Cantidad": Number(row.balance_qty),
      "Saldo Valor": Number(row.balance_value),
    });
  });
  ...
  if (data.length) {
    ...
    rows.push({
      ...
      Entrada: "",
      Salida: "",
      "Saldo Cantidad": Number(last.balance_qty),
      "Saldo Valor": Number(last.balance_value),
    });
